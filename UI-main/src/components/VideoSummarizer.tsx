...
---`;
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res = await apiService.getFeatureHistory('video');
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4 border border-white/20">
                            <div className="flex items-center space-x-2 relative">
                              ...
                              <button
                                type="button"
                                className="ml-2 px-3 py-2 bg-gray-200 rounded-lg text-gray-700 hover:bg-gray-300 border border-gray-300"
                                onClick={fetchHistory}
                                title="Show Question History"
                              >
                                {isLoadingHistory ? <Loader2 className="w-4 h-4 animate-spin" /> : 'History'}
                              </button>
                              {/* History Dropdown/Modal */}
                              {showHistory && (
                                <div className="absolute z-50 left-0 mt-2 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                  <div className="flex justify-between items-center px-3 py-2 border-b border-gray-100">
                                    <span className="font-semibold text-gray-700">Previous Questions</span>
                                    <button className="text-gray-400 hover:text-gray-700" onClick={() => setShowHistory(false)}>
                                      <X className="w-4 h-4" />
                                    </button>
                                  </div>
                                  {history.length === 0 && (
                                    <div className="px-4 py-3 text-gray-500 text-sm">No history found.</div>
                                  )}
                            ...
                            <button
                              ...
                              className="mt-2 w-full bg-confluence-blue/90 backdrop-blur-sm text-white py-2 px-4 rounded-lg hover:bg-confluence-blue disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center space-x-2 transition-colors border border-white/10"
                            >
                              {isQALoading ? (
                                <>
