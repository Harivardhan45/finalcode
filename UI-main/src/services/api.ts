...
export interface CodeImpactRequest {
  space_key: string;
  old_code: string;
  new_code: string;
  question?: string;
...
class ApiService {
  ...
  async codeImpactAnalyzer(request: CodeImpactRequest): Promise<ImpactResponse> {
    return this.makeRequest<ImpactResponse>('/code-impact-analyzer', {
      method: 'POST',
      body: JSON.stringify(request),
    });
  }
