...
async def code_assistant(request: CodeRequest, req: Request):
    ...
    try:
        ...
        code_blocks = []
        # Try to extract Confluence code macro blocks first
        for macro in soup.find_all('ac:structured-macro', {'ac:name': 'code'}):
            plain_text = macro.find('ac:plain-text-body')
            if plain_text and plain_text.text.strip():
                code_blocks.append(plain_text.text)
        # If not found, try <pre> or <code> tags
        if not code_blocks:
            for tag in soup.find_all(['pre', 'code']):
                code_text = tag.get_text()
                if code_text.strip():
                    code_blocks.append(code_text)
        # If still not found, fallback to plain text (no HTML)
        if code_blocks:
